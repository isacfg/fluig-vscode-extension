...
import { FastExportConfigView } from "../views/FastExportConfigView";

export class FormExtension {
    public static activate(context: vscode.ExtensionContext): void {
        context.subscriptions.push(
            vscode.commands.registerCommand(
                "fluiggers-fluig-vscode-extension.fastExportForm",
                FormFastExportService.fastExport // Chama o serviço de exportação rápida
            )
        );
        context.subscriptions.push(
            vscode.commands.registerCommand(
                "fluiggers-fluig-vscode-extension.openFastExportConfig",
                () => {
                    new FastExportConfigView(context).show(); // Abre a tela de configuração
                }
            )
        );
        context.subscriptions.push(
                    ...
                    if (!fileUri) {
                        if (!vscode.window.activeTextEditor) {
                            vscode.window.showErrorMessage(
                                "Não há editor de texto ativo com Formulário"
                            );
                            return;
                        }
    ...
    private static async createForm() {
        ...
        await vscode.workspace.fs.writeFile(
            formUri,
            readFileSync(
                vscode.Uri.joinPath(TemplateService.templatesUri, "form.txt")
                    .fsPath
            )
        );
    ...
    private static async createFormEvent(folderUri: vscode.Uri) {
        ...
        if (!folderUri) {
            if (!vscode.window.activeTextEditor) {
                vscode.window.showErrorMessage(
                    "Não há editor de texto ativo com Dataset"
                );
                return;
            }
        ...
        if (!folderUri.path.includes("/forms/")) {
            vscode.window.showErrorMessage(
                "Necessário selecionar um formulário para criar o evento."
            );
            return;
        }

        const formName: string = folderUri.path.replace(
            /.*\/forms\/([^/]+).*/,
            "$1"
        );
        ...
        await vscode.workspace.fs.writeFile(
            ...
            isNewFunction
                ? Buffer.from(
                      TemplateService.createEmptyFunction(eventName),
                      "utf-8"
                  )
                : readFileSync(
                      vscode.Uri.joinPath(
                          TemplateService.formEventsUri,
                          `${eventName}.txt`
                      ).fsPath
                  )
        );
